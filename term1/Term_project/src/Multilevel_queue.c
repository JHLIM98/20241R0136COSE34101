...
void Multilevel_queue(Process *p[], result* MLQ_result, int time_quantum)
{
    ...
    Queue* ready_queue_high      = init_Queue();      //RR
    ...
    while(size_queue(done_queue) < PROCESS_COUNT)
    {
        ...
        while (!isEmpty_PQ(new_queue) && time >= front_pq(new_queue)->arrived_time)
        {
            ...
            if(front_pq(new_queue)->priority <= 3){
                push_queue(ready_queue_high, pop_pq(new_queue));
            }
            else if(front_pq(new_queue)->priority <= 6 && front_pq(new_queue)->priority > 3){
                push_queue(ready_queue_middle, pop_pq(new_queue));
            }
            else{
                push_queue(ready_queue_low, pop_pq(new_queue));
            }
        }
        // waiting process
        while (!isEmpty_PQ(waiting_queue) && front_pq(waiting_queue)->io_remain_time <= 0)
        {
            ...
            if(front_pq(waiting_queue)->priority <= 3){
                push_queue(ready_queue_high, pop_pq(waiting_queue));
            }
            else if(front_pq(waiting_queue)->priority <= 6 && front_pq(waiting_queue)->priority > 3){
        ...
        if (!isEmpty_queue(run) && run->data[0]->remaining_time == 0)
        {
            ...
            record_result(MLQ_result, &node1);
        ...
        if(!isEmpty_queue(run) && 
            ...
            run->data[0]->io_remain_time != 0)
            {
                ...
                record_result(MLQ_result, &node_wait);
        ...
        {
            if(time_quantum <= time - run->data[0]->resume_time && run->data[0]->priority <= 3) // hiqh que인 경우 RR 나머지는 FCFS
            {
                ...
                push_queue(ready_queue_high, pop_queue(run));
            }

            else if(run->data[0]->priority <= 6 && run->data[0]->priority > 3)      //hugh que task가 아니면 
            {
                strcpy(run->data[0]->state, "ready");
                executed_node node_wait = {run->data[0]->pid, run->data[0]->resume_time, time, "Preemptive"};
                record_result(MLQ_result, &node_wait);
        ...
        if(!isEmpty_queue(run) && !isEmpty_queue(ready_queue_middle) && run->data[0]->priority > 6)
        {
                strcpy(run->data[0]->state, "ready");
                executed_node node_wait = {run->data[0]->pid, run->data[0]->resume_time, time, "Preemptive"};
                record_result(MLQ_result, &node_wait);
